...
/*
// Initialises I²C peripheral i2c_num in slave mode with SDA pin sda_pin, SCL pin scl_pin and a write buffer size of at least buf_size (the size of the read buffer is managed by i2c_slave_set_read_buf).
// When initialised as an I²C slave, the modes of the SDA and SCL pins are changed automatically.
// This function may be used again to change the settings on an initialised I²C peripheral in slave mode.
...
// This callback is called when the I²C write starts and again I²C write completes and/or when the write buffer saturates.
...
// This callback is called once when the I²C read starts and again when the I²C read completes and/or when the read buffer depletes.
...
// The buffer must remain valid until either the I²C slave is de-initialised or the buffer is replaced by another call to i2c_slave_set_read_buf, whichever comes first.
// The position in the read buffer is reset to 0 every time a read is initiated.
void i2c_slave_set_read_buf	(badge_err_t *, int, const uint8_t *, size_t);
*/
